...
Option 0 makes this a type error and forces the user to specify the intended behavior. Option 1[^worst-option] either wraps to 8 bits (even though `53 * 34` fits in 16 bits) or treats the overflow as illegal or traps. Option 2 is what C does: the operation becomes `u16(u32(a) * u32(b))`, preserving the most value information and likely performing better with a natural integer size. Option 3 is the same idea but uses a larger intermediate type such as `u64` or `u128`. Option 4 is my preferred approach if I wanted implicit conversions: propagate the type hint from the left-hand side top-down so that it serves as the widest type down the syntax tree, resulting in `u16(a) * u16(b)` directly.
